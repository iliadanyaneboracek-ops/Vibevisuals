...
public class PvpCombatHudElement extends HudElement {
    ...
    public void render(DrawContext context, MinecraftClient client, float tickDelta, boolean editorMode) {
        ...
        drawScaledText(context, client, hudText(name, false), textX + config.nameXOffset, nameY,
                HudGlass.textPrimary(), config.nameTextScale);
        ...
        drawScaledText(context, client, hudText(stats, false), textX + config.statsXOffset,
                nameY + scaledTextHeight(config.nameTextScale) + 2 + config.statsYOffset,
                HudGlass.textSecondary(), config.statsTextScale);
    ...
    private static void drawItemCounter(
        ...
        drawScaledText(
                context,
                client,
                hudText(Integer.toString(count), false),
                x + config.itemIconSize + config.itemGap + countXOffset,
                y + 1 + countYOffset,
                HudGlass.textPrimary(),
    ...
    private static void drawHealthBar(DrawContext context, int x, int y, int width, int height, float progress) {
        ...
        // Subtle theme-aware track + accent fill.
        HudCardRenderer.drawOverlayCard(context, x, y, width, height, radius,
                MenuTheme.TEXT_NEUTRAL, 0.12f);
        int fillW = Math.round(width * progress);
        if (fillW >= radius * 2) {
            HudCardRenderer.drawOverlayCard(context, x, y, fillW, height, radius,
                    MenuTheme.ACCENT_BRIGHT, 0.92f);
        } else if (fillW > 0) {
            context.fill(x, y, x + fillW, y + height,
                    MenuTheme.withAlpha(MenuTheme.ACCENT_BRIGHT, 0.92f));
        }
    }
